...
export default function CreateCandidatoModal({ isOpen, activeElection, candidatos, onClose, onSuccess }: Props) {
  ...
  useEffect(() => {
    ...
    if (region && !uniqueProvincias.includes(provincia)) setProvincia('');
  }, [region, ubicaciones]);
  ...
  useEffect(() => {
    ...
    if (provincia && !uniqueDistritos.includes(distrito)) setDistrito('');
  ...
  const loadPartidos = async () => {
    ...
    try {
      ...
      setPartidos(data);
      if (data.length > 0) setPartidoId(data[0].id);
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      setError(err.response?.data?.message || 'Error al inscribir el candidato');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleClose = () => {
    ...
    setProvincia('');
    setDistrito('');
    setCargo(CargoCandidato.REGIONAL);
    setFotoFile(null);
    setPreviewUrl(null);
    setError('');
    onClose();
                ...
                {cargo !== CargoCandidato.REGIONAL && (
                    <div>
                        ...
                        <select
                            ...
                            disabled={!region && regiones.length > 0} // Only disable if there are actually regions loaded and one isn't selected
